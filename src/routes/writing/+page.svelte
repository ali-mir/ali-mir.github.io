<script>
  import '../../app.css';
</script>

<svelte:head>
  <title>ali mir / writing</title>
</svelte:head>

<div class="flex justify-center mx-auto">
  <div class="p-5 w-full max-w-2xl">
    <h1 class="text-[#C44B4F] font-bold text-3xl text-center">writing</h1>

    <!-- thoughts -->
    <h2 class="text-[#4C5355] font-bold text-xl mt-8 mb-3">thoughts</h2>
    <ul class="space-y-3">
      <li
        class="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-1 sm:gap-4"
      >
        <a href="/writing/juryduty" class="text-[#536c73] font-bold text-lg"
          >jury duty</a
        >
        <p class="text-[#888888] text-sm sm:text-base uppercase tracking-wide">
          september 14th, 2024
        </p>
      </li>
      <li
        class="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-1 sm:gap-4"
      >
        <a href="/writing/carnegiehall" class="text-[#536c73] font-bold text-lg"
          >carnegie hall</a
        >
        <p class="text-[#888888] text-sm sm:text-base uppercase tracking-wide">
          april 30th, 2024
        </p>
      </li>
      <li
        class="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-1 sm:gap-4"
      >
        <a href="/writing/reading" class="text-[#536c73] font-bold text-lg"
          >back to reading</a
        >
        <p class="text-[#888888] text-sm sm:text-base uppercase tracking-wide">
          january 7th, 2024
        </p>
      </li>
    </ul>

    <!-- links -->
    <h2 class="text-[#4C5355] font-bold text-xl mt-8 mb-3">links</h2>
    <ul class="space-y-3">
      <!-- march 2026 - may 2026 -->
      <li
        class="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-1 sm:gap-4"
      >
        <a
          href="/writing/links8_march2026_may2026"
          class="text-[#536c73] font-bold text-lg">links #8</a
        >
        <p class="text-[#888888] text-sm sm:text-base uppercase tracking-wide">
          may 31st, 2026
        </p>
      </li>
      <!-- december 2025 - february 2026 -->
      <li
        class="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-1 sm:gap-4"
      >
        <a
          href="/writing/links7_dec_2025_feb_2026"
          class="text-[#536c73] font-bold text-lg">links #7</a
        >
        <p class="text-[#888888] text-sm sm:text-base uppercase tracking-wide">
          february 28th, 2026
        </p>
      </li>
      <!-- september 2025 - november 2025 -->
      <li
        class="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-1 sm:gap-4"
      >
        <a
          href="/writing/links6_sept_2025_nov_2025"
          class="text-[#536c73] font-bold text-lg">links #6</a
        >
        <p class="text-[#888888] text-sm sm:text-base uppercase tracking-wide">
          november 30th, 2025
        </p>
      </li>
      <!-- june 2025 - august 2025 -->
      <li
        class="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-1 sm:gap-4"
      >
        <a
          href="/writing/links5_june_2025_aug_2025"
          class="text-[#536c73] font-bold text-lg">links #5</a
        >
        <p class="text-[#888888] text-sm sm:text-base uppercase tracking-wide">
          august 31st, 2025
        </p>
      </li>
      <!-- february 2025 - may 2025 -->
      <li
        class="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-1 sm:gap-4"
      >
        <a
          href="/writing/links4_feb_2025_may_2025"
          class="text-[#536c73] font-bold text-lg">links #4</a
        >
        <p class="text-[#888888] text-sm sm:text-base uppercase tracking-wide">
          may 31st, 2025
        </p>
      </li>
      <!-- november 2024 - january 2025 -->
      <li
        class="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-1 sm:gap-4"
      >
        <a
          href="/writing/links3_nov_2024_jan_2025"
          class="text-[#536c73] font-bold text-lg">links #3</a
        >
        <p class="text-[#888888] text-sm sm:text-base uppercase tracking-wide">
          january 31st, 2025
        </p>
      </li>
      <!-- august 2024 - october 2024 -->
      <li
        class="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-1 sm:gap-4"
      >
        <a
          href="/writing/links2_aug_2024_oct_2024"
          class="text-[#536c73] font-bold text-lg">links #2</a
        >
        <p class="text-[#888888] text-sm sm:text-base uppercase tracking-wide">
          october 31st, 2024
        </p>
      </li>
      <!-- may 2024 - july 2024 -->
      <li
        class="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-1 sm:gap-4"
      >
        <a
          href="/writing/links1_may_2024_july_2024"
          class="text-[#536c73] font-bold text-lg">links #1</a
        >
        <p class="text-[#888888] text-sm sm:text-base uppercase tracking-wide">
          july 31st, 2024
        </p>
      </li>
    </ul>

    <!-- notes -->
    <h2 class="text-[#4C5355] font-bold text-xl mt-8 mb-3">notes</h2>
    <p class="text-[#4C5355] text-base leading-relaxed mb-3">
      short technical notes on software topics i find interesting or want to
      dive deeper on. mostly for my own learning, writing helps me think. very
      raw, little polish!
    </p>
    <ul class="space-y-3">
      <li class="text-[#888888] text-sm italic">coming soon</li>
    </ul>
  </div>
</div>
